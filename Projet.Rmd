...
La question de recherche de ce projet est la suivante : Quelles sont les caractéristiques les plus importantes des différentes races de chien et comment ces caractéristiques peuvent-elles affecter l’adaptabilité de l’animal dans certains contextes spécifiques ? L'objectif de cette analyse est d’aider la population à se renseigner quant aux particularités entourant chaque race de chien et à prendre des décisions éclairées lors de l’adoption.

Pour l’analyse de nos données, nous voulons établir la relation entre les différentes caractéristiques des races, par exemple, la taille et l’intelligence, mais aussi les catégoriser afin de déterminer quelle race convient le mieux à telle personne selon son mode de vie et préférences. Nous jugeons que les chiens ayant besoin de plus d'exercice et qui requièrent plus de soins devraient avoir un propriétaire ayant beaucoup de temps pour gérer tout cela. Nous allons donc déterminer quels sont les types qui ont besoin de ce temps. De plus, on s'attend à ce que les chiens ayant un indice amical plus élevé soient meilleurs avec les enfants, ce qui conviendrait aux familles. On pose aussi comme hypothèse que les chiens ayant une plus grande taille ont une espérance de vie moins élevée, une information que les gens devraient savoir. Finalement, on croit qu'il y a une dépendance entre le niveau d'intelligence du chien et la difficulté à l'entrainer.
...
Si la durée de vie est plus petite (<) que 10 ans, on la définit comme "courte". 
Si elle est plus grande ou égale (>=) à 10 et plus petite ou égale (<=) à 13, alors entre 10 et 13 ans inclusivement, on la définit comme "moyenne". 
Si elle est plus grande (>) que 13, on la définit comme "longue".
...
Si le chien a besoin de plus (>) d'une heure jusqu'à (<=) deux heures d'activité physique par jour, on dit que son besoin est "moderate".
Si le chien a besoin de plus de (>) deux heures d'activité physique par jour, on dit que son besoin est "high".
...
Si, sur l'échelle de 10, son indice d'intelligence est plus petit ou égal (<=) à 6, on dit que son indice est "low".
Si, sur l'échelle de 10, son indice d'intelligence est entre 7 et 8 inclusivement, on dit que son indice est "moderate".
Si, sur l'échelle de 10, son indice d'intelligence est plus grand ou égal (>=) à 9, on dit que son indice est "high".
...
Afin de déterminer quels types de chien requièrent plus de temps, nous allons créer un graphique en barre dans lequel nous inclurons les variables "Grooming_Needs", "Daily_Exercise_Needs_category", "Size" et "Type". Afin de visualiser le tout, nous utiliserons un facet_wrap.
...
Des parents avec des jeunes enfants ou qui aiment être en contact avec des familles devraient se tourner vers ces races de chiens. Les plus haut classés ont les meilleures caractéristiques. Nous avons mis 30 choix pour laisser la chance aux gens de se baser également sur leurs autres goûts personnels. C'est pourquoi nous avons ajouté une caractéristique unique de chaque race dans cette liste.
...
Ici, on peut déduire que l'équation de la pente est y = -0,075x + 13.68. Maintenant, il est simple et facile lorsqu'on connait le poids d'estimer l'espérance de vie. Assez pratique quand les chiens ont atteint leur poids stable. Par exemple, un chien de 1 kg aurait une espérance de vie d'environ 13,068 ans.
